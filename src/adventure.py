...
# ---------------------This will change all paths--------------------
soundsDir = "C:\\Users\\Moises\\Desktop\\GitHub\\Immersive-Text-Game-Project\\resources\\sounds\\"
imagesDir = "C:\\Users\\Moises\\Desktop\\GitHub\\Immersive-Text-Game-Project\\resources\\images\\"

#-------------------------------------------Game Code-------------------------------------------
# Adventure Game Updates
# This is the updated adventure game which asks for user input to move east, west, south, or north
def adventure():
  global name, validMoves, validActions, sounds, messages, images, inven
  # Room sounds
...
def firstStage(image):
  ...
  while move != "exit":
    ...
    # Broken Pots
      ...
      copyInto(images["brokenLose"], image, 0, 0)
      repaint(image)
      play(sounds["brokenLose"])
      showInformation(messages["brokenLose"])
    ...
    # Wooden Doors
      ...
      if inven["flashlight"] == true and inven["batteries"] == true:
        response = askUseFL()
        if response == "true":
          copyInto(images["woodenDoorsWin"], image, 0, 0)
          repaint(image)
          play(sounds["woodenDoors"])
          showInformation(messages["win"])
          stopPlaying(sounds["openClose"])
          stopPlaying(sounds["woodenDoors"])
          copyInto(levelComplete(image), image, 0, 0)
          ...
          move = response
          continue
      copyInto(images["woodenDoorsLose"], image, 0, 0)
      repaint(image)
      play(sounds["woodenLose"])
      showInformation(messages["lose"])
      stopPlaying(sounds["openClose"])
...
def leftStage(image):
  ...
  while move != "exit":
    ...
    # Key Diner
      ...
      if inven["keyDiner"] == true or flashlightOn == false:
    ...
    # See Police
      ...
      if inven["notepad"] == true:
        repaint(image)
        play(sounds["policeCarTook"])
        showInformation(messages["policeCarTook"])
        stopPlaying(sounds["policeCarTook"])
      else:
      ...
      if move == "w":#sideStreet
        ...
        if flashlightOn:
          copyInto(flashlightFilter(images["keypad"]), image, 0, 0)
        else:
          copyInto(images["keypad"], image, 0, 0)
        repaint(image)
        play(sounds["keypad"])
        showInformation(messages["keypad"])
        stopPlaying(sounds["keypad"])
        response = askCode()
        if response == "true":
          play(sounds["correctCode"])
          ...
          play(sounds["wrongCode"])
          showInformation(messages["wrongCode"])
          stopPlaying(sounds["wrongCode"])
    ...
    # Police Station
      ...
      repaint(image)
      play(sounds["policeStation"])
      showInformation(messages["policeStation"])
      stopPlaying(sounds["policeStation"])
      copyInto(images["win2"], image, 0, 0)
      repaint(image)
      play(sounds["win2"])
      showInformation(messages["win2"])
      stopPlaying(sounds["win2"])
      copyInto(levelComplete(image), image, 0, 0)
      play(sounds["levelComplete"])
      repaint(image)
    ...
    # Lose Radio
      ...
      #copyInto(images["radioSound"], image, 0, 0)
      #repaint(image)
      play(sounds["radioSound"])
      showInformation(messages["radioSound"])
      stopPlaying(sounds["radioSound"])
      #copyInto(images["loseRadio"], image, 0, 0)
      #repaint(image)
      play(sounds["loseRadio"])
      showInformation(messages["loseRadio"])
      stopPlaying(sounds["loseRadio"])
      copyInto(gameOver(image), image, 0, 0)
      repaint(image)
      play(sounds["gameOver"])
      showInformation(messages["loser"])
...
def rightStage(image):
  ...
  while move != "exit":
    ...
    if location == 0:
      ...
      repaint(image)
      play(sounds["entryway"])
      showInformation(messages["entryway"])
      stopPlaying(sounds["entryway"])
    ...
    # Factory Bathroom
      ...
      repaint(image)
      play(sounds["openClose"])
      play(sounds["bathroomFactory"])
      showInformation(messages["bathroomFactory"])
      stopPlaying(sounds["bathroomFactory"])
      stopPlaying(sounds["openClose"])
      move = askMove("e", "You can move East.")
      if move == "e":#entryway
        play(sounds["openClose"])
    ...
    # Supply Closet
      ...
      repaint(image)
      play(sounds["openClose"])
      play(sounds["supplyCloset"])
      showInformation(messages["supplyCloset"])
      stopPlaying(sounds["openClose"])
      stopPlaying(sounds["supplyCloset"])
      move = askMove("w", "You can move west.")
      if move == "w":#entryway
        play(sounds["openClose"])
        location -= 2
...
#Blurs an image, makes it more yellow, and adds the text "LEVEL COMPLETE" to the center. 
def levelComplete(pic):
 picCopy = copy(pic)
 levelCompletePic = moveAndFade(pic, picCopy, 10, factor=4)
 levelCompletePic = moreYellow(levelCompletePic, 10)
 levelCompletePic = winText(levelCompletePic, "LEVEL COMPLETE")
 return levelCompletePic

#Moves and fades image
...
#Increases yellow color of pixels by n%.
def moreYellow(pic, n = 50.0):
  n = n*.01
  #Adds a yellow tint to a selected picture
  for p in getPixels(pic):
    r = getRed(p)
    g = getGreen(p)
    b = getBlue(p)
    setRed(p, r + (255 - r) * n)
    setGreen(p, g + (255 - g) * n)
    setBlue(p, b + (0 - b) * n)
  return pic
...
#Adds yellow "LEVEL COMPLETE" text to the center of an image.
def winText(pic, string):
 x = (getWidth(pic)/2)-220
 y = (getHeight(pic)/2)-15
 s = makeStyle(sansSerif, italic+bold, 48)
 addTextWithStyle(pic, x, y, string, s, yellow)
 return pic
...
def getSoundsDict():
  # Room sounds
  start = makeSound(soundsDir+"Long_Note_Three.wav")
  openClose = makeSound(soundsDir+"openClose.wav")
  intro = makeSound(soundsDir+"intro.wav")
  bedroom = makeSound(soundsDir+"bedroom.wav")
  bedroomSouth = makeSound(soundsDir+"bedroomSouth.wav")
  bedroomNorth = makeSound(soundsDir+"bedroomNorth.wav")
  closet = makeSound(soundsDir+"closet.wav")
  closetFoundFL = makeSound(soundsDir+"closetFoundFL.wav")
  hallSouth = makeSound(soundsDir+"hallSouth.wav")
  hallNorth = makeSound(soundsDir+"hallNorth.wav")
  lockedDoor = makeSound(soundsDir+"lockedDoor.wav")
  lock = makeSound(soundsDir+"lock.wav")
  useKey = makeSound(soundsDir+"useKey.wav")
  bathroom = makeSound(soundsDir+"bathroom.wav")
  bathroomFoundKey = makeSound(soundsDir+"bathroomFoundKey.wav")
  bathroomGotKey = makeSound(soundsDir+"bathroomGotKey.wav")
  masterBedroom = makeSound(soundsDir+"masterBedroom.wav")
  stairs = makeSound(soundsDir+"stairs.wav")
  foyer = makeSound(soundsDir+"foyer.wav")
  balcony = makeSound(soundsDir+"balcony.wav")
  balconyClose = makeSound(soundsDir+"balconyClose.wav")
  archway = makeSound(soundsDir+"archway.wav")
  archwayFoundBatteries = makeSound(soundsDir+"archwayFoundBatteries.wav")
  brokenLose = makeSound(soundsDir+"brokenLose.wav")
  woodenDoors = makeSound(soundsDir+"woodenDoors.wav")
  woodenLose = makeSound(soundsDir+"woodenLose.wav")
  gameOver = makeSound(soundsDir+"gameOver.wav")
  startNext = makeSound(soundsDir+"startNext.wav")
  leftIntro = makeSound(soundsDir+"leftIntro.wav")
  leftIntro2 = makeSound(soundsDir+"leftIntro2.wav")
  creakDiner = makeSound(soundsDir+"creakDiner.wav")
  diner = makeSound(soundsDir+"diner.wav")
  backToCar = makeSound(soundsDir+"backToCar.wav")
  radio = makeSound(soundsDir+"radio.wav")
  keyDiner = makeSound(soundsDir+"keyDiner.wav")
  keyDinerGotKey = makeSound(soundsDir+"keyDinerGotKey.wav")
  sideStreet = makeSound(soundsDir+"sideStreet.wav")
  seePolice = makeSound(soundsDir+"seePolice.wav") 
  policeCar = makeSound(soundsDir+"policeCar.wav")
  policeCarTook = makeSound(soundsDir+"policeCarTook.wav")
  keypad = makeSound(soundsDir+"keypad.wav")
  wrongCode = makeSound(soundsDir+"wrongCode.wav")
  correctCode = makeSound(soundsDir+"correctCode.wav")
  policeStation = makeSound(soundsDir+"policeStation.wav")
  win2 = makeSound(soundsDir+"win2.wav")
  radioSound = makeSound(soundsDir+"radioSound.wav")
  loseRadio = makeSound(soundsDir+"loseRadio.wav")
  rightIntro = leftIntro
  rightIntro2 = makeSound(soundsDir+"rightIntro2.wav")
  entryway = makeSound(soundsDir+"entryway.wav")
  factory = makeSound(soundsDir+"factory.wav")
  supplyCloset = makeSound(soundsDir+"supplyCloset.wav")
  bathroomFactory = makeSound(soundsDir+"bathroomFactory.wav")
  outside = makeSound(soundsDir+"outside.wav")
  logs = makeSound(soundsDir+"logs.wav")
  jump = makeSound(soundsDir+"jump.wav")
  hide = makeSound(soundsDir+"hide.wav")
  crawl = makeSound(soundsDir+"crawl.wav")
  wait = makeSound(soundsDir+"wait.wav")
  levelComplete = makeSound(soundsDir+"levelComplete.wav")
  dict = {"start":start, "openClose": openClose, "closet":closet, "closetFoundFL":closetFoundFL, "bedroomSouth":bedroomSouth,
    ...
    "leftIntro2":leftIntro2, "creakDiner":creakDiner, "diner":diner, "backToCar":backToCar, "radio":radio, "keyDiner":keyDiner,
    "keyDinerGotKey":keyDinerGotKey, "rightIntro":rightIntro, "rightIntro2":rightIntro2, "sideStreet":sideStreet,
    "seePolice":seePolice, "policeCar":policeCar, "policeCarTook":policeCarTook, "keypad":keypad,
    "wrongCode":wrongCode, "correctCode":correctCode, "policeStation":policeStation, "win2":win2,
    "radioSound":radioSound, "loseRadio":loseRadio, "rightIntro":rightIntro, "rightIntro2":rightIntro2,
    "entryway":entryway, "factory":factory, "supplyCloset":supplyCloset, "bathroomFactory":bathroomFactory,
    "outside":outside, "logs":logs, "jump":jump, "hide":hide, "crawl":crawl, "wait":wait,
...
def getImagesDict():
  # Room Images
  bedroom = makePicture(imagesDir+"bedroom.jpg")
  bedroomSouth = makePicture(imagesDir+"bedroomSouth.jpg")
  bedroomNorth = makePicture(imagesDir+"bedroomNorth.jpg")
  closet = makePicture(imagesDir+"closet.jpg")
  closetTook = makePicture(imagesDir+"closetTook.jpg")
  closetFoundFL = makePicture(imagesDir+"closetFoundFL.jpg")
  hallSouth = makePicture(imagesDir+"hallSouth.jpg")
  hallNorth = makePicture(imagesDir+"hallNorth.jpg")
  bathroom = makePicture(imagesDir+"bathroom.jpg")
  bathroomTook = makePicture(imagesDir+"bathroomTook.jpg")
  bathroomFoundKey = makePicture(imagesDir+"bathroomFoundKey.jpg")
  bathroomFoundKeyTook = makePicture(imagesDir+"bathroomFoundKeyTook.jpg")
  masterBedroom = makePicture(imagesDir+"masterBedroom.jpg")
  foyer = makePicture(imagesDir+"foyer.jpg")
  foyerTook = makePicture(imagesDir+"foyerTook.jpg")
  balcony = makePicture(imagesDir+"balcony.jpg")
  archway = makePicture(imagesDir+"archway.jpg")
  archwayTook = makePicture(imagesDir+"archwayTook.jpg")
  brokenLose = makePicture(imagesDir+"brokenLose.jpg")
  woodenDoorsLose = makePicture(imagesDir+"woodenDoorsLose.jpg")
  woodenDoorsWin = makePicture(imagesDir+"woodenDoorsWin.jpg")
  backToCar = makePicture(imagesDir+"backToCar.jpg")
  diner  = makePicture(imagesDir+"diner.jpg")
  dinerTook = makePicture(imagesDir+"dinerTook.jpg")
  keyDiner = makePicture(imagesDir+"keyDiner.jpg")
  keyDinerTook = makePicture(imagesDir+"keyDinerTook.jpg")
  keypad = makePicture(imagesDir+"keypad.jpg")
  leftIntro2 = makePicture(imagesDir+"leftIntro2.jpg")
  padlock = makePicture(imagesDir+"padlock.jpg")
  policeCar = makePicture(imagesDir+"policeCar.jpg")
  policeStation = makePicture(imagesDir+"policeStation.jpg")
  sideStreet = makePicture(imagesDir+"sideStreet.jpg")
  sideStreetTransition = makePicture(imagesDir+"sideStreetTransition.jpg")
  startNext = makePicture(imagesDir+"startNext.jpg")
  radio = makePicture(imagesDir+"radio.jpg")
  radioTook = makePicture(imagesDir+"radioTook.jpg")
  bathroomFactory = makePicture(imagesDir+"bathroomFactory.jpg")
  crawl = makePicture(imagesDir+"crawl.jpg")
  entryway = makePicture(imagesDir+"entryway.jpg")
  factory = makePicture(imagesDir+"factory.jpg")
  hide = makePicture(imagesDir+"hide.jpg")
  jump = makePicture(imagesDir+"jump.jpg")
  logs = makePicture(imagesDir+"logs.jpg")
  outside = makePicture(imagesDir+"outside.jpg")
  policeStation = makePicture(imagesDir+"policeStation.jpg")
  supplyCloset = makePicture(imagesDir+"supplyCloset.jpg")
  supplyClosetTook = makePicture(imagesDir+"supplyClosetTook.jpg")
  wait = makePicture(imagesDir+"wait.jpg")
  rightIntro2 = makePicture(imagesDir+"rightIntro2.jpg")
  win2 = makePicture(imagesDir+"win2.jpg")
...
def getMessagesDict():
  ...
  supplyCloset = "\n-----Supply Closet-----\n"\
    ...
    +"them. As you look around for anything useful you see some batteries but you already have some, so you "\
    +"leave them there."
  bathroomFactory = "\n-----Factory Bathroom-----\n"\
  ...
  hide = "You decide to hide behind the large boulder. As you reach the boulder you quickly slip around and crouch "\
    +"behind it, while covering your head. You hear logs crashing around you and slamming into the boulder, which "\
